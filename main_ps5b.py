...
# Set the chief of the spacecraft. Enable maneuvers for SC3.
...
sc3.forces['maneuvers'] = True # Important to switch this on.
...
while timeNow < duration:
    ...
    # Record states.
    ROE = np.array([sc3.da, sc3.dL, sc3.ex, sc3.ey, sc3.ix, sc3.iy])
    roe_history[k,:] = ROE
    state_history[k,:] = [sc3.pR, sc3.pT, sc3.pN, sc3.vR, sc3.vT, sc3.vN]
    ephem_history[k,:] = [sc3.px, sc3.py, sc3.pz, sc3.vx, sc3.vy, sc3.vz]
        ...
        if (flag_reconfig_1[0] == True) and (flag_reconfig_1[1] == True):
            if (flag_reconfig_1[2] == False):
                if reconfig_timer_1 == None:
                    ...
                    sc3.vy = sc3.vy + dv[1]
                    sc3.vz = sc3.vz + dv[2]
                    reconfig_timer_1 = sc3.chief.a * dROE[1] / (3 * dv_lambda)
                    total_deltaV += dv_lambda
                elif (reconfig_timer_1 < 0) or (abs(dROE[1]) < 0.002):
                    print("t = ", timeNow, '\n')
        ...
        if (flag_reconfig_2[0] == True) and (flag_reconfig_2[1] == True):
            if (flag_reconfig_2[2] == False):
                if reconfig_timer_2 == None:
                    ...
                    sc3.vy = sc3.vy + dv[1]
                    sc3.vz = sc3.vz + dv[2]
                    reconfig_timer_2 = sc3.chief.a * dROE[1] / (3 * dv_lambda)
                    total_deltaV += dv_lambda
                elif (reconfig_timer_2 < 0) or (abs(dROE[1]) < 0.002):
                    print("t = ", timeNow, '\n')
                    print("Completed drift correction! \n")
                    print("dROE = ", dROE, '\n')
...
plt.show()
desiredROE12 = plt.scatter([dep2_ROE_1[1], dep2_ROE_2[1], dep2_ROE_3[1]], 
...
plt.show()
desiredROE34 = plt.scatter([dep2_ROE_1[2], dep2_ROE_2[2], dep2_ROE_3[2]], 
...
plt.show()
desiredROE56 = plt.scatter([dep2_ROE_1[4], dep2_ROE_2[4], dep2_ROE_3[4]], 
